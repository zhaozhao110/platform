package com.editor.tool.project.create
{
	...
	//command
	public class CreateCommandTool
	{
		public function CreateCommandTool()
		{
		}
		...
		public function create(evt_name:String,fold_sign:String):void
		{
			...
			var path:String = ProjectCache.getInstance().getCommandPath() + File.separator + "action" + File.separator+ clsName+".as"
			...
			if(cont.indexOf(sign1) == -1){
				SandyManagerBase.getInstance().showError("AppStartUpCommand类里没有registerAllCommand函数");
				return ;
			}
			
			cont = getTemple(12);
			cont = StringTWLUtil.replace(cont,"TempCommand",clsName);
			
			write.write(file,cont);
			
			var new_fl:File = new File(ProjectCache.getInstance().getCommandPath()+File.separator+"action");
			...
			cont = before_s + NEWLINE_SIGN;
			cont += createSpace(4)+"registerCommand("+eventFile.name.split(".")[0]+'.'+eventName+','+clsName+");"+NEWLINE_SIGN;
			cont += after_s;
			write.write(fl,cont);
		}
		
		private var sign1:String = "registerAllCommand():void{"
