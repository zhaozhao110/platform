package com.editor.module_gdps.view.publish.mediator
{
	...
	public class GdpsPublishDataGridMediator extends AppMediator
	{
		public static const NAME:String = "GdpsPublishDataGridMediator";
		...
		public function GdpsPublishDataGridMediator(viewComponent:Object = null, _confItem:AppModuleConfItem = null)
		{
		...
		public function get publish_grid():SandyDataGrid
		{
			return dataGridContainer.tableSpace_grid;
		}
		public function get publish_page_bar():GdpsPageBar
		{
		...
		override public function onRegister():void
		{
			...
			publish_page_bar.pageNoChangeFun = onPageChange;
		...
		private function showDataGridColumn(a:*):void
		{
			...
			var cols:Array = publish_grid.columns || [];
			...
			publish_grid.columns = cols;
			
			findPublishDataByPage(1, publish_page_bar.pageLimit, null);
		}
		...
		{
			var len:int = publish_grid.dataProvider.length;
			var index:int = 0;
			for(var i:int = 0 ;i < len ;i++)
			{
				var obj:Object = publish_grid.dataProvider[i] as Object;
				if(obj == oItem){
					index = i;
					break;
				}
		 ...
		 * 从服务器端加载表数据
		 */
		public function findPublishDataByPage(pageNo:int, pageLimit:int, params:*):void
		{
			var http:GdpsHttpServiceLocator = new GdpsHttpServiceLocator();
			http.args2 = { "pageNo": pageNo, "pageLimit": pageLimit, "SState": "3,4,5,6,0" };
			http.sucResult_f = showPublishData;
			var _url:String = GDPSServices.getBatchRecord_url;
			http.conn(_url, "POST" );
		}
		
		private function showPublishData(a:*):void
		{
			var value:* = a;
			
			publish_grid.dataProvider = value.data;
			
			publish_page_bar.pageData = value.data.length;
			publish_page_bar.totalCount = value.page.totalRowCount;
			publish_page_bar.setBtnStatus();
		}
		
		//当页数发生变化时的处理函数
		private function onPageChange(pageNo:int, pageLimit:int):void
		{
			findPublishDataByPage(pageNo, pageLimit, null);
		}
	}
}