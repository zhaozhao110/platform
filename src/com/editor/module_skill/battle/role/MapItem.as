	...
	public class MapItem extends SandyMapItem2
	{
		...
		override protected function parserMixMotion():void
		{
			...
			for(var i:int=0;i<item.list.length;i++)
			{
				...
				var urlSign:String = SandyMapConst.getURLSign(MapItemData(data).getActionSign("",imap),rend.actionType);
		...
		override protected function getMapSourceData():SandyMapSourceData
		{
			var d:SandyMapSourceData = new SandyMapSourceData();
			d.sceneType = getSceneType();
			return d;
		}
		
		override public function getAnimationData():SandyMapSourceData
		{
			var d:SandyMapSourceData = new SandyMapSourceData();
			d.sceneType = getSceneType();
			if(checkIsMaster()){
