package com.editor.d3.pop.preTexture
{
	...
	//在material选择texture,specualerMap
	public class App3DPreTextureWin extends UIVBox
	{
		public function App3DPreTextureWin()
		{
			super();
			create_init();
		}
		...
		private function create_init():void
		{
			...
			tile.verticalGap = 15;
			tile.itemRenderer = App3DPreTextureWinItemRenderer;
			addChild(tile);
			...
			openBtn.label = "图片编辑器打开"
			openBtn.addEventListener(MouseEvent.CLICK , onOpenImg);
			v.addChild(openBtn);
		...
		private function onOkBtn(e:MouseEvent=null):void
		{
			var f:File = tile.selectedItem as File;
			if(f == null) return ;
			if(confirm_f!=null) confirm_f(f,BitmapDataUtil.cloneBitmap(img.content as Bitmap));
			visible = false;
		}
		...
		{
			onResize()
			var a:Array = D3ProjectFilesCache.getInstance().getAllTextureAssets();
			tile.dataProvider = a;
			dataChange = false;
			visible = true;
		}
		...
		private function onTileChange(e:ASEvent):void
		{
			if(e.isDoubleClick){
				...
				var f:File = tile.selectedItem as File;
				if(f == null) return ;
				if(confirm_f!=null) confirm_f(f,BitmapDataUtil.cloneBitmap(App3DPreTextureWinItemRenderer(e.component).getBitmap()));
				visible = false;
				
				return ;
			}
			f = tile.selectedItem as File;
			img.source = f;
